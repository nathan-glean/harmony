...
use crate::models::{Repo, Session, Ticket, Worktree};
use crate::now_unix;
...
impl Store {
    ...
    pub async fn add_ticket(
        ...
        let now = now_unix();
        let status = if !spec.trim().is_empty() && repo_id.is_some() {
            crate::status::READY
        } else {
    ...
    /// Set the agent spec; promote available → ready since the ticket is now actionable.
    pub async fn set_ticket_spec(&self, id: i64, spec: &str) -> Result<()> {
        sqlx::query(
            "UPDATE tickets
             SET spec = ?, status = CASE WHEN status = 'available' THEN 'ready' ELSE status END,
                 updated_at = ?
             WHERE id = ?",
        )
        .bind(spec)
