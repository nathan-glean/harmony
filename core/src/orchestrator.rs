...
pub fn answer_question(
    ...
    let opts = options
        ...
        .collect::<Vec<_>>()
        .join("\n");
    let cardinality = if multi_select {
        "one or more"
    } else {
        "exactly one"
    };
    ...
    match tag.to_ascii_uppercase().as_str() {
        "ANSWER" => {
            ...
            if selected.is_empty() {
                QDecision::Escalate {
                    reason: "unparseable answer".into(),
                }
            } else {
                QDecision::Answer {
                    selected,
        ...
        "CUSTOM" if !rest.trim().is_empty() => QDecision::Answer {
            selected: vec![],
            custom: Some(rest.trim().to_string()),
        },
        "ESCALATE" => QDecision::Escalate {
            reason: rest.trim().to_string(),
        },
        _ => QDecision::Escalate {
            reason: "unparseable conductor output".into(),
        },
    }
}
...
mod tests {
    ...
    fn parse_q_custom() {
        assert_eq!(
            ...
            QDecision::Answer {
                selected: vec![],
                custom: Some("use Postgres with a JSONB column".into())
            }
        );
    }
    ...
    fn parse_spec_accept_and_escalate() {
        ...
        assert_eq!(parse_spec("  accept  "), SpecDecision::Accept);
        assert!(matches!(
            parse_spec("ESCALATE changes scope"),
            SpecDecision::Escalate { .. }
        ));
        // Fail-safe.
        assert!(matches!(
