...
/// Ask `claude -p` (read-only, on `model`) whether the incremental `diff` warrants re-review and/or
/// re-proof. Fail-safe: on any spawn/parse failure the caller should fall back to [`Decision::FULL`].
pub fn triage(worktree: &str, diff: &str, model: &str) -> Result<Decision> {
    let prompt = format!(
...
/// Decide re-verification for the incremental change: heuristic fast-path, else the LLM triage (on
/// `model`), falling back to the full pipeline if the LLM call fails.
pub fn decide(worktree: &str, changed: &[String], diff: &str, model: &str) -> Decision {
    if let Some(d) = heuristic(changed) {
        return d;
    }
    triage(worktree, diff, model).unwrap_or(Decision::FULL)
}
